/* ===============================================================
   DDL Script : Load Script 
   =============================================================== 
   Tujuan Script :
      Skrip Ini bertujuan untuk membuat store_procedure yang digunakan
	untuk me masukan masing masing  tabel yang sudah dilakukan 
	transformasi dari bronze layer ke dalam silver layer dan juga
	memasukan mencatatnya ke dalam silver.etl_log
   =============================================================== */
    Noted :
     	Sebelum membuat store procedure dan transform lakukan lah 
	data understanding dan pengcekan dalam nilai null dan duplikat
   =============================================================== */
...
BEGIN
    ...
    -- CRM_CUST_INFO
    -- =============================================================
    start_time := clock_timestamp(); -- Waktu Mulai -- 
    RAISE NOTICE 'Loading silver.crm_cust_info...';

	-- Hapus tabel jika ada --
    DROP TABLE IF EXISTS silver.crm_cust_info;

	-- Membuat tabel dan masukan table sesuai dengan transformasinya -- 
    ...
    end_time := clock_timestamp(); -- waktu akhir --
    duration := end_time - start_time; -- durasi proces --
    RAISE NOTICE 'crm_cust_info loaded in %', duration;

	-- Kumpulkan statistik -- 
	ANALYZE silver.crm_cust_info;

	-- table_name -- 
	select distinct tablename from pg_stats where tablename = 'crm_cust_info' into v_table_name;

	-- total_baris --
	select count(*) from silver.crm_cust_info into v_total_row ;

	--total_column -- 
	select count(attname) from pg_stats where tablename = 'crm_cust_info' and schemaname = 'silver' into v_total_column;
	
	raise notice '%',v_total_column;
	raise notice 'Table % selesai',v_table_name;
	raise notice 'Table dimasukan ke dalam silver.etl_log';

	-- masukan hasil log ke dalam table log --
	insert into silver.etl_log(table_name,total_column,total_row,proces_time,load_date) 
	values (v_table_name,v_total_column,v_total_row,duration,now());

	-- pengecekan apakah hasil log berhasil di masukan --
	perform 1 table_name from silver.etl_log where table_name = 'crm_cust_info';
	if found then 
		raise notice 'Tabel hasil log berhasil dimasukan';
	else
		raise notice 'Tabel hasil log gagal dimasukan ';
	end if;
